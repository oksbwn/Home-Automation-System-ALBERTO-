...
function charts(sensor) {
	...
	/* ---------- Chart with points ---------- */
	/*-----------------Modified By Me Author: Bikash Narayan Panda-------------------------------------------------*/
			...
			$("#y").text(pos.y.toFixed(2));
				...
				if (item) {
					if (previousPoint != item.dataIndex) {
						...
						showTooltip(item.pageX, item.pageY,"Date:  " + x + " : " + y);
					}
				}
				else {
					$("#tooltip").remove();
	...
	if($("#sensors_data").length)
	{
		//var visitors = [[1, 50], [2, randNum()-10], [3, randNum()-10], [4, randNum()],[5, randNum()],[6, 4+randNum()],[7, 5+randNum()],[8, 6+randNum()],[9, 6+randNum()],[10, 8+randNum()],[11, 9+randNum()],[12, 10+randNum()],[13,11+randNum()],[14, 12+randNum()],[15, 13+randNum()],[16, 14+randNum()],[17, 15+randNum()],[18, 15+randNum()],[19, 16+randNum()],[20, 17+randNum()],[21, 18+randNum()],[22, 19+randNum()],[23, 20+randNum()],[24, 21+randNum()],[25, 14+randNum()],[26, 24+randNum()],[27,25+randNum()],[28, 26+randNum()],[29, 27+randNum()], [30, 31+randNum()]];
		var xmlDoc=loadXMLDoc("http://192.168.0.1/smart_home/API/web_interface/get_sensor_data_with_id.php?ID="+sensor);
		var sensorsData=[];
		var sensorsTime=[];
		for(i=60;i>0;i--){
		try{
				sensorsData[i]=[i,xmlDoc.getElementsByTagName("M"+(60-i))[0].getAttribute("data")];
				sensorsTime[i]=xmlDoc.getElementsByTagName("M"+(60-i))[0].getAttribute("time");
			}catch(error){sensorsData[i]=[i,0];}
		}
		//alert(expenseData[1]);
		var plot = $.plot($("#sensors_data"),
			   [ { data: sensorsData, label: "DATA" } ], {
				   series: {
					   lines: { show: true,
								lineWidth: 3,
								fill: true, fillColor: { colors: [ { opacity: 0.5 }, { opacity: 0.2 } ] }
		...
		function showTooltip(x, y, contents) {
			$('<div id="tooltip">' + contents + '</div>').css( {
				...
				opacity: 1
			}).appendTo("#sensors_data").fadeIn(200);
		}

		var previousPoint = null;
		$("#sensors_data").bind("plothover", function (event, pos, item) {
			$("#x").text(pos.x.toFixed(2));
			$("#y").text(pos.y.toFixed(2));
				...
				if (item) {
					if (previousPoint != item.dataIndex) {
						...
						var x = item.datapoint[0].toFixed(2),
							y = item.datapoint[1].toFixed(2);
						showTooltip(item.pageX, item.pageY,"Time: " + sensorsTime[Math.abs(x)] + " Value: " + y);		
					}
				}
				else {
					$("#tooltip").remove();
